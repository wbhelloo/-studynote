## Diary

### 常用工具命令

- [00_Markdown_简明语法](./00_Markdown-Commands.md)
- [01_Linux_常用命令](./00_Linux-Commands.md)
- [02_Conda_常用命令](./00_Conda-Commands.md)


### 学习笔记 

- [01_pipreqs-查找python项目依赖包并生成requirements.txt](./01_Python-Commond-pipreqs.md)
- [02_Linux常用命令: tar、zip 压缩和解压缩命令](./02_Linux-Command-zip.md)
- [03_Win10安装Tensorflow-gpu方法](./03_Win10-Install-Tensorflow-GPU.md)
- [04_汽车车牌模糊处理代码](./04_Mask-License-Plate.md)
- [05_深度学习常用激活函数Python实现](./05_Activation-Function.md)
- [06_linux程序设置后台运行及查看方法](./06_linux-Setup-code-runs-in-the-background.md)
- [07_Jupyter-Notebook-远程访问设置](./07_Jupyter-Notebook-Remote-access-Settings.md)
- [08_conda update conda 时报错](./08_Conda-update-update-error.md)

- [09_分享Conda虚拟环境到Anaconda-Cloud](./09_Conda_share_an_env_on_cloud.md)